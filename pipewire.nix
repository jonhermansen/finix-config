...
assert ! udevSupport -> ! enableSystemd;
...
stdenv.mkDerivation (finalAttrs: {
  ...
  patches = [
    ...
    (path + "/0060-libjack-path.patch")
    # Move installed tests into their own output.
    (path + "/0070-installed-tests-path.patch")
  ];
  ...
  mesonFlags = [
    ...
    (lib.mesonEnable "udev" (!enableSystemd && udevSupport && stdenv.hostPlatform.isLinux))
